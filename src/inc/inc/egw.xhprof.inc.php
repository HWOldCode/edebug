...
// add Patch to modi url rendering
// @author Stefan Werfling


/**
 * xhprof_render_link_source
 * @param type $target
 * @param type $base_path
 * @return type
 */
function xhprof_render_link_source($target, $base_path) {
	if( isset($GLOBALS['XHPROF_URLS']) ) {
		if( is_array($GLOBALS['XHPROF_URLS']) && isset($GLOBALS['XHPROF_URLS'][$target]) ) {
			return $GLOBALS['XHPROF_URLS'][$target];
		}
	}
...
function profiler_report ($url_params,
  ...
  $top_link_query_string = xhprof_render_link_source('?', $base_path) . http_build_query($base_url_params);
  ...
  if ($diff_mode) {
    ...
    $run1_link = xhprof_render_link('View Run #' . $run1,
                           xhprof_render_link_source('?', $base_path) .
...
function print_function_info($url_params, $info, $sort, $run1, $run2) {
  ...
  $href = xhprof_render_link_source('?', $base_path) .
...
function full_report($url_params, $symbol_tab, $sort, $run1, $run2) {
  ...
  if ($diff_mode) {
    ...
    $base_url_params = xhprof_array_unset(xhprof_array_unset($url_params,
                                                             'run1'),
                                          'run2');
    $href1 = xhprof_render_link_source('?', $base_path) .
      http_build_query(xhprof_array_set($base_url_params,
                                        'run', $run1));
    $href2 = xhprof_render_link_source('?', $base_path) .
      http_build_query(xhprof_array_set($base_url_params,
                                        'run', $run2));
  ...
  print("<center><br><h3>" .
        xhprof_render_link($callgraph_report_title,
                    xhprof_render_link_source('callgraph.php?', $base_path) . http_build_query($url_params))
        . "</h3></center>");
  ...
  if (strncmp($info['fn'], 'run_init', 8) && $info['fn'] !== 'main()') {
    if (defined('XHPROF_SYMBOL_LOOKUP_URL')) {
      $link = xhprof_render_link(
        'source',
        xhprof_render_link_source(XHPROF_SYMBOL_LOOKUP_URL .'?', '') . 'symbol='.rawurlencode($info["fn"]));
      print(' ('.$link.')');
    }
  }
}
